...
var elem = function () {
    ...
    this.setAttribute("onclick", 'location.href="page_game.html"'); //назначает onclick переход на страницу с игрой
    // this.setAttribute("onmouseup", 'mouseLeave.call(this.parentNode)');
}
...
var changeCard = function (){
    ...
    for(var i = 1; i<=4; i++){
        ...
        if(parentEl.children[i] != this){ //просматриваем все карточки кроме нажатой
            ...
            parentEl.children[i].setAttribute("onclick", 'elem.call(this)');//возвращает нажатие на div "part" ссылка на метод
            countMove += 5;
        }
    }
}
...
var mouseLeave = function (){
    ...
    for(var i = 1; i<=4; i++){
        ...
        this.children[i].setAttribute("onclick", 'elem.call(this)');
